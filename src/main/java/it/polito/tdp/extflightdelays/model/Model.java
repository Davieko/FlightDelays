...
public class Model {
	...
	//creo grafo a parte perché ogni volta l'utente clicca sul bottone, ripartiamo da un grafo da 0
	public void creaGrafo(int x) {
		...
		for(Rotta r : dao.getRotte(idMap)) {
			if(this.grafo.containsVertex(r.getA1()) && this.grafo.containsVertex(r.getA2())) {
				//recupero l'arco, in cui getEdge prende l'arco a prescindere dal verso perché non orientato
		...
		System.out.println("# Vertichi: "+grafo.vertexSet().size());
	...
	public List<Airport> trovaPercorso(Airport a1, Airport a2){
		...
		it.addTraversalListener(new TraversalListener<Airport, DefaultWeightedEdge>(){
			...
			public void edgeTraversed(EdgeTraversalEvent<DefaultWeightedEdge> e) {
				...
				Airport airport2 = grafo.getEdgeSource(e.getEdge());
				
				if(visita.containsKey(airport1) && !visita.containsKey(airport2)) {
					//quindi a2 è scoperto da a1
